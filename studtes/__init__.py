...
class Player(BasePlayer):
    ...
    age = models.IntegerField(label="2. Сколько Вам полных лет?",
                              )
    ...
    educ_par = models.StringField(label="4. Какое утверждение наиболее точно описывает финансовое положение Вашей семьи?",
    ...
    q14 = models.StringField(
        label="8. Основная причина низкого дохода – лень?",
        choices=['Да', 'Нет'],
        widget=widgets.RadioSelect
    )
    ...
    q19 = models.StringField(
        label="24. Представьте, что Вы положили 100 000 рублей на банковский вклад на 2 года под 10% годовых. По условиям договора капитализация процентов отсутствует. Как Вы думаете, сколько денег принесет вклад за второй год: больше, чем в первый год, столько же или меньше?",
        choices=['Больше',
                 'Столько же',
                 'Меньше',
                 'Не знаю'],
    ...
    q20 = models.StringField(
        label="25. Какой из годовых депозитов выгоднее для сбережения денег? ",
    ...
    q21 = models.StringField(
        label="26. Как Вы считаете, верно ли утверждение, что обычно покупка акций одной компании обеспечивает большую надежность вложения, чем покупка доли в паевом инвестиционном фонде? ",
        choices=['Да',
            'Нет',
    ...
    q23 = models.StringField(
        label="28. Какая ставка рефинансирования в настоящий момент? ",
        choices=['8-10%',
                 '11-13%',
                 '14-16%',
                 '17-21%',
    ...
    q24 = models.StringField(
        label="29. Сколько составляла ставка рефинансирования в декабре 2023 года? ",
        choices=['8-10%',
                 '11-13%',
                 '14-16%',
                 '17-21%',
    ...
    q25 = models.StringField(
        label="30. Знаете ли вы, что такое ИИС?",
        choices=['Да', 'Нет'],
        widget=widgets.RadioSelect
    )
    q26 = models.StringField(
        label="31. Имеете ли вы опыт инвестирования в финансовые инструменты (например, акции, облигации), фонды (ПИФы, ETF), банковские вклады или другие виды вложений?",
        choices=['Да', 'Нет'],
        widget=widgets.RadioSelect
    )
    ...
    q29 = models.StringField(
        label="33. Когда человек может выйти на пенсию?",
    ...
    q30 = models.StringField(
        label="34. Какой минимальный рабочий стаж требуется для получения страховой пенсии?",
        choices=['1 год', '11 лет', '15 лет', '25 лет'],
        widget=widgets.RadioSelect
    )
    q31 = models.StringField(
        label="35. Что такое страховые взносы?",
    ...
    q32 = models.StringField(
        label="36. Какой процент от страховых взносов идет на пенсионные отчисления?",
        choices=['0,1', '0,13', '0,22'],
        widget=widgets.RadioSelect
    )

    q48 = models.StringField(
        label="37. Как Вам кажется, кто должен заботиться о том, чтобы у человека было достаточно средств для поддержания достойного уровня жизни на пенсии: скорее государство или скорее сам человек?",
    ...
    q50 = models.StringField(
        label="38. Как часто Вы думаете о своем выходе на пенсию?",
        choices=['Много', 'Иногда', 'Немного', 'Совсем не думаю'],
        widget=widgets.RadioSelect
    )

    q51 = models.StringField(
        label="39. Если бы Вы могли выбирать, в каком возрасте Вы бы хотели выйти на пенсию?",
        choices=['30-40 лет',
                 '40-50 лет',
                 '50-60 лет',
                 '60-70 лет',
    ...
    q70 = models.StringField(
        label="40. Задумывались ли Вы из чего будет формироваться Ваша пенсия?",
        choices=['Да', 'Нет'],
        widget=widgets.RadioSelect
    )
...
class Page5(Page):
    form_model = "player"
    form_fields = ['q38', 'q39', 'q40', 'q41', 'q42', 'q43', 'q44', 'q45', 'q46', 'q47']
...
page_sequence = [MyPage, Page2, Page6, Page5, Page3, Page4,  Results]
